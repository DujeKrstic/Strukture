...
Position FindBySurname(Position first, char* surname) {
    ...
    Position temp = first; //iako je first poslan necemo s njim radit nego inicijaliziramo temp=first

    //  Ako su dva stringa ista, onda strcmp() vraca 0, ako nisu, vraca broj razlicit od nule
    //  strcmp() usporeduje stringove karakter po karakter po ASCII vrijednostima karaktera
    ...
    /*
        ...
        prezimenom , ako ga nademo, returnamo adresu strukture (odnosno temp koji je tipa Position koji je tipa Person *) 
        na kojoj je prezime nadeno ako se prezime ne nade vracamo NULL
    */
    return NULL;
}
...
Position FindBefore(Position wantedPerson, Position head) {
    ...
    // ako je sljedeci nakon temp-a (temp->next) onaj kojeg zelimo obrisat, onda bas taj temp trebamo i njega vracamo funkcijom return
